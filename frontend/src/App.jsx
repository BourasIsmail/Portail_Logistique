...
import { useAuth, getInfoFromToken, getTokenFromCookie } from "./utils/AuthProvider"
  ...
  return (
    <>
      <Route
        path="/md/dashboard"
        element={
          <ProtectedRoute role={["ROLE_ADMIN", "ROLE_INFO", "ROLE_LOGISTICS", "ROLE_USER"]}>
      ...
      <Route
        path="/md/*"
        element={
          <ProtectedRoute role={["ROLE_USER", "ROLE_INFO", "ROLE_LOGISTICS", "ROLE_ADMIN"]}>
...
function gestionMarcheRoutes() {
  return (
    ...
    >
      {/* Nested routes that will display inside GMMainPage's <Outlet /> */}
...
function gestionParcAutoRoutes() {
  return (
    ...
    >
      ...
      <Route path="chauffeurs/modifier/:chauffeurId" element={<ChauffeurEditForm />} />
      <Route path="missions" element={<MissionsList />} />
      <Route path="missions/ajouter" element={<MissionForm />} />
      <Route path="missions/modifier/:missionId" element={<MissionEditForm />} />
      ...
      <Route path="moyens-paiement/modifier/:moyenPaiementId" element={<MoyenPaiementEditForm />} />
      <Route path="depenses/modifier/:depenseId" element={<DepenseEditForm />} />
      <Route path="*" element={<Navigate to="/parc-auto/vehicules" replace />} />
    </Route>
  )
}
  ...
  return (
    <>
      <ThemeProvider defaultTheme="light" storageKey="vite-ui-theme">
        <NotificationProvider>
          <Routes>
            ...
            <Route
              path="/"
              element={
                userDetails && (userDetails?.role == "ROLE_ADMIN" || userDetails?.role == "ROLE_LOGISTICS") ? (
                  <MainPage />
                ) : userDetails?.role == "ROLE_DIR" ? (
                  <Navigate to="/gm/tableau-de-bord" />
                ) : (
                  <Navigate to="/md/dashboard" />
                )
              }
        ...
        <Toaster richColors visibleToasts={1} duration={2000} position="top-center" style={{ marginTop: "1rem" }} />
      </ThemeProvider>
    </>
  )
}
