                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {vehicules.map((vehicule) => (
                            <tr key={vehicule.id} className="hover:bg-gray-50">
                                <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{vehicule.immatriculation}</td>
                                <td className="px-6 py-4 whitespace-nowrap">{vehicule.marque} {vehicule.modele}</td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                        vehicule.statut === 'EN_SERVICE' ? 'bg-green-100 text-green-800' :
                                        vehicule.statut === 'EN_MISSION' ? 'bg-blue-100 text-blue-800' :
                                        'bg-red-100 text-red-800'
                                    }`}>
                                        {vehicule.statut}
                                    </span>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">{vehicule.kilometrage} km</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <Button variant="ghost" size="icon" className="mr-2 hover:bg-blue-100">
