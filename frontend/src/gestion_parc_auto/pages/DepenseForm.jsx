...
export default function DepenseForm() {
  ...
  const [depenseData, setDepenseData] = useState({
    typeDepense: "", // Le champ clé pour la logique dynamique
  ...
  useEffect(() => {
    const loadDependencies = async () => {
      try {
        const [vehiculesRes, moyensPaiementRes] = await Promise.all([
          getAllVehiculesForSelect(),
          getAllMoyensPaiementForSelect(),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // ... (logique de soumission avec parseInt)
        ...
        <Card className="max-w-4xl mx-auto border-gray-300 shadow-lg">
          <CardHeader className="bg-slate-50 border-b p-6">
            ...
            <CardDescription className="text-slate-500">
              Enregistrez une dépense liée à un véhicule.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-8 space-y-12">
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <Select
                    name="typeDepense"
                    onValueChange={(v) => handleSelectChange("typeDepense", v)}
                    required
                  >
                    <SelectTrigger>
            ...
            {/* Section pour Plein de Carburant */}
            ...
            {/* Section pour Recharge Jawaz */}
            {depenseData.typeDepense === "RechargeJawaz" && (
              <section className="space-y-6 animate-in fade-in-50">
                <div className="flex items-center space-x-3 border-b pb-3">
                  <Zap className="h-7 w-7 text-yellow-500" />
                  <h3 className="text-xl font-semibold">
                    Détails de la Recharge Jawaz
                  </h3>
                </div>
                <p className="text-sm text-gray-600">
                  Aucun détail supplémentaire n'est requis pour ce type de
                  dépense.
                </p>
              </section>
            )}
